...
for i in type_disea:
  ...
  # os.makedirs(os.path.join('/content/dataset_split/valid/', i), exist_ok=True)
...
def split_data_files(X, Y):
  ...
  # for i in range(len(X_valid)): shutil.copy(os.path.join(os.path.join(data, y_valid[i]),X_valid[i]), os.path.join('/content/dataset_split/valid', y_valid[i]))
...
def make_balance(df):
  ...
  for name_disea in type_disea:
    ...
    # print("so luong", len(disea_df), name_disea)
    d.append(disea_df)
...
for i, (train_idx, valid_idx) in enumerate(k.split(train_df['filepaths'], train_df['labels'])):
  a = make_balance(train_df.loc[train_idx])
  b = make_balance(train_df.loc[valid_idx])
